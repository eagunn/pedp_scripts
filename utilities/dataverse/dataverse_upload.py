"""
Simple Dataverse File Uploader
...
Usage:
    ...
    python dataverse_upload.py --url https://dataverse.harvard.edu --token YOUR_TOKEN --pid doi:10.7910/DVN/XXXXX --dir C:/path/to/files
"""
...
# Skip archive files (set to True to avoid uploading archives)
SKIP_ARCHIVES = True  # Set to False if you want to upload .zip, .gz, .7z, etc.

# Archive extensions to skip if SKIP_ARCHIVES is True
ARCHIVE_EXTENSIONS = ['.zip', '.gz', '.7z', '.tar', '.rar', '.bz2', '.xz', '.tgz']
...
def main():
    ...
    all_files = [f for f in Path(upload_dir).iterdir() if f.is_file()]

    # Filter out archive files if SKIP_ARCHIVES is True
    files_to_upload = []
    skipped_archives = []

    for f in all_files:
        file_ext = f.suffix.lower()
        if SKIP_ARCHIVES and file_ext in ARCHIVE_EXTENSIONS:
            skipped_archives.append(str(f))
        else:
            files_to_upload.append(str(f))

    if skipped_archives:
        print(f"\n⚠️  Skipping {len(skipped_archives)} archive files:")
        for archive in skipped_archives[:5]:  # Show first 5
            print(f"    - {os.path.basename(archive)}")
        if len(skipped_archives) > 5:
            print(f"    ... and {len(skipped_archives) - 5} more")
        print()

    if not files_to_upload:
        print(f"\n✗ No files to upload (all files were archives or directory is empty)")
        return
