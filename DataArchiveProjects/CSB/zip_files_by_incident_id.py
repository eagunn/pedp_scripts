#!/usr/bin/env python3
"""
Zip CSB Investigation Files by Incident ID

This script groups CSB investigation files by their incident ID (the prefix before
...
Usage:
    python zip_files_by_incident_id.py [--mode {both,zip,log}]
...
Configuration:
    - Source directory: D:\data_archives_2025\CSB\Past\Investigations
    - Output directory: D:\data_archives_2025\CSB\Past\Investigations\zips_by_id_for_upload
    - Skip log: zip_script_log.txt (in source directory)
    - CSV log: zip_archives_inventory.csv (in output directory)
"""
...
# Configuration
SOURCE_DIR = r"D:\data_archives_2025\CSB\Past\Investigations"
OUTPUT_DIR = r"D:\data_archives_2025\CSB\Past\Investigations\zips_by_id_for_upload"
LOG_FILE = "zip_script_log.txt"
CSV_LOG = "zip_archives_inventory.csv"

def main():
    """Main function to zip files by incident ID."""

    # Parse command line arguments
    parser = argparse.ArgumentParser(
        description='Zip CSB investigation files by incident ID and/or generate inventory CSV'
    )
    parser.add_argument(
        '--mode',
    ...
    if args.mode in ['both', 'zip']:
        ...
        for incident_id, files_list in sorted(incident_files.items()):
            zip_filename = os.path.join(OUTPUT_DIR, f"{incident_id}.zip")

            print(f"Creating {incident_id}.zip ({len(files_list)} files)")
